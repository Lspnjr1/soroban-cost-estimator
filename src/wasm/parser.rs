...
/// Counts a WASM module's sections and collects its custom section names.
///
/// Iterates the binary payload-by-payload, counting every structural and
/// custom section and recording the name of each custom section (e.g.
/// `contractspecv0`). Used to surface module structure in reports — no
/// per-section size breakdown is computed (out of scope by design).
...
pub struct SectionInfo {
    /// Total number of sections in the module (structural + custom).
    pub count: usize,
    /// Names of all custom sections, e.g. `contractspecv0` or `name`.
...
pub struct WasmInfo {
    ...
    pub has_spec: bool,
    /// Total WASM binary size in bytes.
    pub size: usize,
    /// Section layout metadata (count + custom section names).
